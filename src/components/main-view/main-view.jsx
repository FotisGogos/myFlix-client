...
class MainView extends React.Component {
  ...
  getUser(token) {
    ...
    axios
      .get(`https://moviexperts.herokuapp.com/users/${username}`, {
  ...
  getMovies(token) {
    axios
      .get("https://moviexperts.herokuapp.com/movies", {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
  ...
  render() {
    ...
    const { user, username, email, password, birthday, favoriteMovies } =
      this.state;
    return (
      <Router>
        {/* Main view */}
              ...
              render={({ match, history }) => {
                ...
                return (
                  <Col md={8}>
                    <MovieView
                      movie={movies.find(
                        (m) => m.title === match.params.movieTitle
                      )}
                      onBackClick={() => history.goBack()}
                    />
                  </Col>
